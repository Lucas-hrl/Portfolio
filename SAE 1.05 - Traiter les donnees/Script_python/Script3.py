...
if __name__ == "__main__":
    ...
    liste_legendes = []
    for num_page in range((NB_MAXI_FICHIERS + NB_LEGENDES_PAR_PAGE - 1) // NB_LEGENDES_PAR_PAGE):
        start = num_page * NB_LEGENDES_PAR_PAGE
        stop = min(start + NB_LEGENDES_PAR_PAGE, NB_MAXI_FICHIERS)
        fichiers_page = liste_fichiers[start:stop]
        couleurs_page = liste_couleurs[start:stop]
        legende = Legendes(fichiers_page, couleurs_page, 0, len(fichiers_page))
